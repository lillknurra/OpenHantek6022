#!/usr/bin/env python3
"""Patch 003 - apply architecture documentation.

Creates docs/architecture/DESIGN_DECISIONS.md and verifies that
SYSTEM_ARCHITECTURE.md is already present.

This script is intentionally documentation-only and idempotent.
"""
...
SYSTEM_ARCH = ROOT / "docs" / "architecture" / "SYSTEM_ARCHITECTURE.md"
DESIGN_DECISIONS = ROOT / "docs" / "architecture" / "DESIGN_DECISIONS.md"
...
def main() -> None:
    ...
    if not SYSTEM_ARCH.exists():
        raise SystemExit(f"ERROR: required file missing: {SYSTEM_ARCH.relative_to(ROOT)}")

    DESIGN_DECISIONS.parent.mkdir(parents=True, exist_ok=True)

    desired = CONTENT.rstrip() + "\n"
    if DESIGN_DECISIONS.exists():
        existing = DESIGN_DECISIONS.read_text(encoding="utf-8")
        if existing == desired:
            print("PASS: docs/architecture/DESIGN_DECISIONS.md already has expected content")
            return
        raise SystemExit(
            "ERROR: docs/architecture/DESIGN_DECISIONS.md exists with different content; review it manually before overwriting"
        )

    DESIGN_DECISIONS.write_text(desired, encoding="utf-8")
    print("PASS: created docs/architecture/DESIGN_DECISIONS.md")
    print("PASS: Patch 003 architecture apply complete")
